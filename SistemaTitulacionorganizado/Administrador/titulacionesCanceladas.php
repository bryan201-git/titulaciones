...
if(isset($_SESSION['estaLogueado'])){
	...
	$titulacionescanceladas = "select alumnos.noncontrol, alumnos.nombre, apellidopaterno, apellidomaterno, maestro.nombre as docente, nombretema,motivo
		from titulacion
		inner join alumnos on titulacion.noncontrol = alumnos.noncontrol
		inner join maestro on titulacion.asesormaestro = maestro.rfc
		inner join cancelaciones on titulacion.idtitulacion = cancelaciones.idtitulacion
		where titulacion.status=\"c\"";
	...
	if (!$resultado = $db->query($titulacionescanceladas)) {
		echo "<p align=center>Error al cargar las titulaciones</p>";
	} else {
